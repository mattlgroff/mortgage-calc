...
function App() {
  ...
  const [yearsOfMortgage, setYearsOfMortgage] = useState(defaultYearsOfMortgage);
  ...
  const [propertyTaxPercentage, setPropertyTaxPercentage] = useState(defaultPropertyTaxPercentage);
  ...
  const [homeownersInsuranceYearlyCost, setHomeownersInsuranceYearlyCost] = useState(defaultHomeownersInsuranceYearlyCost);

  useEffect(() => {
    localStorage.setItem('HOMEOWNERS_INSURANCE_YEARLY_COST', homeownersInsuranceYearlyCost);
  ...
  const monthlyPaymentShouldBeAtOrBelow = useMemo(() => {
    return Number(takeHomePay * 0.25).toFixed();
  }, [takeHomePay]);
  ...
  return (
    <div className="App">
      ...
      <div className="input-group">
        <label htmlFor="take-home-pay">Take Home Pay</label>
        <input
          type="number"
          id="take-home-pay"
          value={takeHomePay}
      ...
      <div className="input-group">
        <label htmlFor="homeowners-insurance-yearly-cost">Homeowners Insurance Yearly Cost</label>
          <input
            type="number"
      ...
      <h2>Your monthly payment should not exceed <strong>${monthlyPaymentShouldBeAtOrBelow}</strong></h2>
      <p>That's 25% of your take home pay</p>
